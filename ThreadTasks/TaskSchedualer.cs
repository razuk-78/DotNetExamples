...
class Scheduler
{
   ...
   // this make only one task run at the time;
    public static void test()
    {
       
            var sc = new LimitedConcurrencyLevelTaskScheduler(1);
        Task.Factory.StartNew(() =>
        {
            for (int i = 0; i < 10; i++)
            {
                Thread.Sleep(100);
                Console.WriteLine($"from first: {Thread.CurrentThread.ManagedThreadId}");
            }
        }, new CancellationToken(), TaskCreationOptions.None, sc);
   
        Task.Factory.StartNew(() => { while (true) { Thread.Sleep(1000); Console.WriteLine($"from second: {Thread.CurrentThread.ManagedThreadId}"); } }, new CancellationToken(), TaskCreationOptions.None, sc);
...
public class LimitedConcurrencyLevelTaskScheduler : TaskScheduler
{
    ...
    public IEnumerable<Task> GetScheduledTasksa()
    {
